...
class Utils {
   ...
   private function sqlerror() {
      if(!empty($this->sql->error)) {
         die(sprintf('<p class="lead text-danger"><strong>SQL ERROR:</strong> %s -- <span class="text-muted">%s</span></p>',$this->sql->error,$this->q));
      } else return true;
   } /* }}} */
   //devuelve el siguiente minuto 5
   function siguiente($minuto=0) {
      $numeros= range(0,55,5);
      $este = 0;
      foreach($numeros AS $numero) { if($minuto<=$numero) { return $this->pad($numero); } }
      return $this->pad($este);
   }
   function pad($t,$s=2) {
      return str_pad($t, $s , 0, STR_PAD_LEFT);
   }
}
